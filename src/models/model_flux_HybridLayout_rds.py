...
def hack_model_attn_w_mask(model: FluxTransformer2DModel, attn_type="interact"):
    ...
    def crop_each_layer(hidden_states, list_layer_box, scale_factor):
        """
        ...
        for layer_idx in range(layer_num):
            ...
            if list_layer_box[layer_idx] == None:
                ...
                print(f"scale_factor in transformer: {scale_factor}.\n")
                x1, y1, x2, y2 = list_layer_box[layer_idx]
                x1, y1, x2, y2 = x1 // 16, y1 // 16, x2 // 16, y2 // 16
                if layer_idx > 0:
                    x1, y1, x2, y2 = math.floor(x1 * scale_factor), math.floor(y1 * scale_factor), math.ceil(x2 * scale_factor), math.ceil(y2 * scale_factor)
                layer_token = layer_states[:, y1:y2, x1:x2, :]
                bs, h, w, c = layer_token.shape
                layer_token = layer_token.reshape(bs, -1, c)
                token_list.append(layer_token)
                if layer_idx > 0:
                    token_len_list.append(h * w)
        ...
        return result, img_segs, token_len_list
    ...
    def fill_in_processed_tokens(hidden_states, full_hidden_states, list_layer_box, token_len_list, scale_factor):
        """
        ...
        """
        used_token_len = 0
        bs = full_hidden_states[0].shape[0]
        layer_num = len(full_hidden_states)
        ...
        for layer_idx in range(layer_num):
            if list_layer_box[layer_idx] == None:
                ...
                full_hidden_states[layer_idx][:, y1:y2, x1:x2, :] = hidden_states[:, used_token_len: used_token_len + token_len_list[layer_idx - 1], :].reshape(bs, y2-y1, x2-x1, -1)
                used_token_len += token_len_list[layer_idx - 1]
        return full_hidden_states

    def unpatchify(hidden_states, n_frames, height, width, channel_latent, scale_factor):
        """
            hidden_states: [1, h1xw1 + h2xw2 + ... + hlxwl , inner_dim]
            full_hidden_states: [1, f, h, w, inner_dim]
            list_layer_box: List, length=f, each element is a Tuple of 4 elements (x1, y1, x2, y2)
        """
        # Assuming `token_len_list` contains the token lengths for each frame (excluding the first frame).
        # Initialize a list to store the extracted hidden states for each frame.
        bs = hidden_states.shape[0]
        hidden_states_list = []
        # Start index for slicing hidden_states.
        start_idx = 0
        ...
        for i in range(n_frames):
            if i == 0:
                # The first frame has a known token length, which can be calculated as:
                first_frame_token_len = (height // 2) * (width // 2)
                # Process the first frame.
                first_frame_hidden_states = hidden_states[:, start_idx : start_idx + first_frame_token_len, :]  # [bs, first_frame_token_len, c_latent*4]
                first_frame_hidden_states = first_frame_hidden_states.view(
                    bs, height // 2, width // 2, channel_latent, 2, 2
                )  # Reshape to [bs, h/2, w/2, c_latent, 2, 2]
                first_frame_hidden_states = first_frame_hidden_states.permute(0, 3, 1, 4, 2, 5)  # [bs, c_latent, h/2, 2, w/2, 2]
                first_frame_hidden_states = first_frame_hidden_states.reshape(bs, channel_latent, height, width)  # [bs, c_latent, h, w]
                hidden_states_list.append(first_frame_hidden_states)
                start_idx += first_frame_token_len  # Update the start index.
            else:
                # The first frame has a known token length, which can be calculated as:
                frame_token_len = int(height // 2 * scale_factor) * int(width // 2 * scale_factor)
                # Process the first frame.
                frame_hidden_states = hidden_states[:, start_idx : start_idx + frame_token_len, :]  # [bs, first_frame_token_len, c_latent*4]
                frame_hidden_states = frame_hidden_states.view(
                    bs, int(height // 2 * scale_factor), int(width // 2 * scale_factor), channel_latent, 2, 2
                )  # Reshape to [bs, h/2, w/2, c_latent, 2, 2]
                frame_hidden_states = frame_hidden_states.permute(0, 3, 1, 4, 2, 5)  # [bs, c_latent, h/2, 2, w/2, 2]
                frame_hidden_states = frame_hidden_states.reshape(bs, channel_latent, int(height * scale_factor), int(width * scale_factor))  # [bs, c_latent, h, w]
                hidden_states_list.append(frame_hidden_states)
                start_idx += frame_token_len  # Update the start index.
    ...
    def new_forward(
        ...
        bs, channel_latent_o, height_o, width_o = hidden_states[0].shape
        n_frames = len(hidden_states)
        layer_hidden_states_list = []
        for i in range(n_frames):
            layer_hidden_states = hidden_states[i]
            bs, channel_latent, height, width = layer_hidden_states.shape
            layer_hidden_states = layer_hidden_states.view(bs, channel_latent, height // 2, 2, width // 2, 2)  # [bs, c_latent, h/2, 2, w/2, 2]
            layer_hidden_states = layer_hidden_states.permute(0, 2, 4, 1, 3, 5) # [bs, f, h/2, w/2, c_latent, 2, 2]
            layer_hidden_states = layer_hidden_states.reshape(bs, height // 2, width // 2, channel_latent * 4) # [bs, f, h/2, w/2, c_latent*4]
            layer_hidden_states = self.x_embedder(layer_hidden_states)
            layer_hidden_states_list.append(layer_hidden_states)

        full_hidden_states_list = []
        for i in range(n_frames):
            layer_hidden_states = layer_hidden_states_list[i]
            full_hidden_states_list.append(torch.zeros_like(layer_hidden_states))
        ...
        layer_hidden_states_with_layer_pe_list = []
        for i in range(n_frames):
            layer_hidden_states = layer_hidden_states_list[i]
            layer_hidden_states = layer_hidden_states + layer_pe[:, i]
            layer_hidden_states_with_layer_pe_list.append(layer_hidden_states)

        hidden_states, img_segs, token_len_list = crop_each_layer(layer_hidden_states_with_layer_pe_list, list_layer_box, scale_factor)  # [bs, L, inner_dim]
        ...
        hidden_states = fill_in_processed_tokens(hidden_states, full_hidden_states_list, list_layer_box, token_len_list, scale_factor)  # [bs, f, h, w, inner_dim]
        
        layer_num = len(hidden_states)
        hidden_states_list = []
        for i in range(layer_num):
            layer_states = hidden_states[i].view(bs, -1, self.inner_dim)  # [bs, f * full_len, inner_dim]
            hidden_states_list.append(layer_states)

        hidden_states = torch.cat(hidden_states_list, dim=1)  # [bs, f * full_len, inner_dim]
        ...
        # unpatchify
        output = unpatchify(hidden_states, n_frames, height_o, width_o, channel_latent_o, scale_factor)  # [bs, f, c_latent, h, w]
